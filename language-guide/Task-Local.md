...
작업은 우선순위(priority)나 취소 상태와 같은 작업 컨텍스트를 가지고 있으며, 상위 작업(Parent Task)의 컨텍스트는 하위 작업(Child Task)에게 자연스럽게 전파됩니다. 이처럼 작업은 기본적으로 컨텍스트를 상속하는 구조를 가지고 있으며, 이 구조를 확장하여 개발자가 추가적인 메타데이터도 함께 전달할 수 있도록 한 것이 바로 태스크-로컬입니다. 
 ...
 태스크-로컬은 작업 컨텍스트 간에 메타데이터(이하 '값')를 안전하고 일관되게 전달합니다. 태스크-로컬 값은 작업 생성이 생성될 때 암시적으로 전달되며, 해당 작업이 생성하는 모든 하위 작업에서도 읽을 수 있습니다. 태스크-로컬 값을 특정 스코프에 바인딩하면, 해당 스코프 내의 동기 함수(Synchronous Function), 비동기 함수(Asynchronous Function), `Task`, 작업 그룹(TaskGroup) 그리고 `async-let` 구문에서도 바인딩된 값을 읽을 수 있습니다. 이 값은 바인딩된 스코프
...
또한, 태스크-로컬을 명시적인 매개변수 전달을 대체하는 수단으로 남용해서는 안 됩니다. 숨겨진 인자(hidden argument)를 통해 값을 전달하면 코드의 동작을 이해하고 추적하기 어려워질 수 있습니다. 명시적인 함수 매개변수를 사용하는 전통적인 방식이 코드 가독성과 유지보수성 측면에서 훨씬 더 우수합니다.
{% endhint %}
...
태스크-로컬은 필요한 값을 검색할 때, 태스크-로컬 스택 전체를 복사하는 방식 대신, 링크드 리스트(Linked List) 기반 검색 방식을 채택했습니다. 

이는 태스크-로컬 값을 실제로 읽는 작업(Task)이 전체 작업 중 일부에 불과하기 때문입니다. 일반적으로 하나의 최상위 작업이 태스크-로컬 값을 바인딩하고, 그 생명 주기 동안 수백 개의 하위 작업이 생성됩니다. 이들 하위 작업 중 일부만 태스크-로컬 값에 접근하며, 많은 작업은 값을 읽지 않습니다. 이러한 특성으로 인해, 모든 하위 작업에 대해 태스크-로컬 스택을 복사하는 것은 메모리 사용량과 작업 생성 비용 측면에서 비효율적입니다. 결과적으로, 값 검색 시 약간의 비용을 감수하는 편이 전체 시스템 성능 측면에서 훨씬 유리합니다.

또한, 이러한 구조를 고려할 때, 태스크-로컬 값을 바인딩하지 않은 ’빈 작업(empty task)’을 검색 과정에서 건너뛰는 최적화는 검색 비용을 낮추고, 검색 성능을 더욱 향상시키는 데 실질적인 이점을 제공합니다.
{% endhint %}
...
<!---->
<!--구조화된 동시성(Structured Concurrency)에서는 작업(Task)의 생명 주기가 스코프(scope)에 국한되며, 모든 작업이 완료되어야만 해당 스코프를 벗어날 수 있습니다. 이 구조 덕분에 하위 작업(child task)은 상위 작업(parent task)의 태스크-로컬(task-local) 값을 안정적으로 참조할 수 있습니다. 상위 작업이 중간에 값을 해제하거나 제거할 경우, 예기치 않은 크래시나 버그가 발생할 수 있기 때문입니다.-->
<!---->
<!--Swift는 다음과 같은 보장을 기반으로 안전성을 유지합니다:-->
<!--    •    스코프가 종료될 때까지, 모든 자식 작업(child task)은 완료되거나, 완료되지 않았다면 자동으로 대기(await)하게 됩니다.-->
<!--    •    스코프가 오류(error)를 던지면서 종료될 경우에도, 모든 자식 작업은 자동으로 취소(cancel)된 후 완료됩니다.-->
<!---->
<!--이러한 구조화된 동시성 모델 덕분에, Task-local 값의 내부 상태를 별도의 락(lock)이나 복잡한 동기화 메커니즘 없이 안전하게 관리할 수 있습니다.-->
<!---->
<!--또한, 이 보장을 통해 자식 작업은 부모 작업(또는 상위 부모 작업)의 태스크-로컬 스택(stack)의 현재 헤드(head)를 직접 참조할 수 있으며, 이를 위해 추가적인 관리 작업(housekeeping)을 따로 구현할 필요가 없습니다.-->
<!---->
<!--Swift는 다음을 확실히 보장합니다:-->
<!--    •    자식 작업은 withValue 블록 내에서 정의된 Task-local 값을 참조할 수 있으며, 부모 작업은 해당 값을 보유한 상태로 유지됩니다.-->
<!--    •    모든 자식 작업은 withValue가 반환되기 전에 반드시 완료됩니다.-->
<!---->
<!--이러한 구조를 통해 Swift는 withValue 함수가 반환될 때 Task-local 값 스택에 바인딩(bound)된 값을 안전하게 제거(pop)할 수 있습니다. 이는 스코프 종료 시점에 모든 자식 작업이 완료되어 있기 때문에, 더 이상 Task-local 값을 참조하고 있는 작업이 존재하지 않음을 보장하기 때문입니다.-->
<!---->
<!--따라서, withValue 함수 스코프 안에서 설정된 Task-local 값은 단순히 클로저가 끝날 때 없어지는 것이 아니라, 해당 스코프 내에서 생성된 모든 하위 작업이 완료될 때까지 유지됩니다. withValue는 명시적인 스코프를 정의하며, 이 범위 내에서만 값이 유효합니다. 스코프가 종료되면 값도 자동으로 제거되어, 태스크-로컬 값을 안전하고 예측 가능하게 사용할 수 있으며, 메모리 누수를 방지할 수 있습니다.-->

<!--구조화된 동시성은 작업의 생명 주기가 스코프에 국한되며, 작업이 모두 끝나야 해당 스코프를 벗어날 수 있다. 이 구조 덕분에 하위 작업은 상위 작업의 태스크-로컬 값을 안정적으로 참조할 수 있음. 상위 작업이 중간에 값을 해제해버리면 예상치 못한 크래시나 버그가 생길 수 있기 때문임. -->
...
<!--withValue 함수 스코프 안에서 설정된 태스크-로컬 값은 단순히 해당 클로저가 끝날 때 없어지는 게 아님. 그 스코프 안에서 생성된 모든 하위작업이 끝날 때까지 유지됨. withValue를 통해 스코프를 만들고, 그 범위 내에서만 값이 유효함. 스코프가 끝나면 값도 자동으로 사라짐. 이 덕분에 태스크-로컬 값을 보다 안정적으로 읽고, 예측 가능하며, 메모리 누수가 일어나는 걸 방지할 수 있음-->
